...
class HomeForm(FlaskForm):
    ...
    radio_group = RadioField('label', choices=[('ISBN','ISBN'),('Title','Title'),('Author','Author'),('Year','Year')], validators=[DataRequired()])
...
class RegistrationForm(FlaskForm):
    ...
    def validate_username(self, username):
        ...
        if user:
            raise ValidationError('That Username is taken. Please choose a diffrent Username')
    
    def validate_email(self, email):
        user = Customer.query.filter_by(email=email.data).first()
        if user:
            raise ValidationError('That Email is taken. Please choose a diffrent Email.')
